...
export async function POST(request: Request) {
  try {
    ...
    if (!profileResponse.ok) {
      ...
      return NextResponse.json({ error: 'Failed to fetch user profile' }, { status: profileResponse.status });
    }
    ...
    if (!createKeyResponse.ok) {
      ...
      return NextResponse.json({ error: 'Failed to create API key' }, { status: createKeyResponse.status });
    }
