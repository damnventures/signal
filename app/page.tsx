...
const HomePage = () => {
  ...
  useEffect(() => {
    if (fetchedOriginalLinks.length > 1) {
      Object.keys(playerRefs.current).forEach(videoId => {
        const player = playerRefs.current[videoId];
        const activeVideoId = getYouTubeVideoId(fetchedOriginalLinks[activeVideoIndex]);
        if (player && videoId !== activeVideoId) {
          try {
            // Check if player is in a state where it can be paused
            if (player.getPlayerState() === 1) { // 1 = playing
              player.pauseVideo();
            }
          } catch (e) {
            console.warn(`[YouTube] Could not pause video ${videoId}`, e);
          }
        }
      });
    }
  }, [activeVideoIndex, fetchedOriginalLinks]);

  const handlePrevVideo = () => {
    setActiveVideoIndex(prevIndex => Math.max(0, prevIndex - 1));
  };

  const handleNextVideo = () => {
    setActiveVideoIndex(prevIndex => Math.min(fetchedOriginalLinks.length - 1, prevIndex + 1));
  };
                ...
                <div className="tv-bezel">
                    ...
                    <div className="window-content video-window-content">
                      <div className="video-embed-container">
                        {fetchedOriginalLinks.map((link, index) => {
                          ...
                          if (videoId) {
                            return (
                              <div key={index} className="youtube-video-wrapper" style={{ display: index === activeVideoIndex ? 'block' : 'none' }}>
                                <iframe
                            ...
                            return (
                              <p key={index} className="main-text" style={{ display: index === activeVideoIndex ? 'block' : 'none' }}>
                  ...
                  <div className="tv-controls">
                    {fetchedOriginalLinks.length > 1 && (
                      <div className="video-navigation-controls" style={{textAlign: 'center', margin: '5px 0'}}>
                        <button onClick={handlePrevVideo} disabled={activeVideoIndex === 0} style={{margin: '0 5px'}}>&lt;</button>
                        <span>{activeVideoIndex + 1} / {fetchedOriginalLinks.length}</span>
                        <button onClick={handleNextVideo} disabled={activeVideoIndex === fetchedOriginalLinks.length - 1} style={{margin: '0 5px'}}>&gt;</button>
                      </div>
                    )}
...
export default HomePage; "Chars" if no token exists
          }),
        });
